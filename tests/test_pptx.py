...
class PPTXTester:
    ...
    def test_image_operations(self) -> None:
        ...
        try:
            ...
            img_id = self.manager.insert_image(1, test_image, Coordinates(x=2, y=2, width=1, height=1))
    ...
    def test_chart_operations(self) -> None:
        ...
        try:
            chart_data = SchemaChartData(
                categories=['Q1', 'Q2', 'Q3', 'Q4'], series=[{'name': 'Sales', 'values': [100, 200, 150, 300]}]
            )

            chart_id = self.manager.create_chart(1, 'bar', chart_data, Coordinates(x=3, y=3, width=4, height=3))

            # Modify chart data
            new_data = SchemaChartData(
                categories=['Q1', 'Q2', 'Q3', 'Q4'], series=[{'name': 'Sales', 'values': [150, 250, 200, 350]}]
            )
    ...
    def test_table_operations(self) -> None:
        ...
        try:
            # Create table
            table_id = self.manager.create_table(1, 3, 3, Coordinates(x=4, y=4, width=3, height=2))
