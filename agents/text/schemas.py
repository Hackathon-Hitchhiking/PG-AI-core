...
class BaseTextConfig(BaseModel):
    ...
    device: DeviceType = Field(default=DeviceType.AUTO, description='Аппаратная платформа для выполнения вычислений')
...
class HFConfig(BaseTextConfig):
    ...
    model_name: str = Field(..., min_length=3, examples=['gpt-3.5-turbo'])
    torch_dtype: Literal['auto', 'float16', 'float32'] = 'auto'
    revision: str | None = None
    tokenizer_name: str | None = None
    context_length: int = 4096
    pipeline_kwargs: dict[str, Any] = Field(default_factory=dict, description='Дополнительные параметры для пайплайна')
...
class APIConfig(BaseTextConfig):
    ...
    api_base: str = Field(..., min_length=8, examples=['https://api.example.com/v1/generate'])
    api_key: str = Field(..., min_length=32, description='API ключ для доступа к API')
    headers: dict[str, str] = Field(default_factory=dict, description='Кастомные заголовки HTTP-запроса')
...
class LangChainConfig(BaseTextConfig):
    ...
    model_name: str = Field(..., min_length=3, examples=['gpt-3.5-turbo'])
    langchain_template: str = Field(default='{input}', min_length=1, examples=['{input}'])
    context_length: int = 4096
...
class LlamaCppConfig(BaseTextConfig):
    ...
    model_path: str = Field(..., min_length=1, examples=['/path/to/llama/model'])
    context_length: int = 4096
    n_gpu_layers: int = -1
    main_gpu: int = 0
    tensor_split: list[float] | None = None
    llamacpp_params: dict[str, Any] = Field(default_factory=dict, description='Дополнительные параметры для LlamaCpp')
...
class LlamaIndexConfig(BaseTextConfig):
    ...
    model_name: str = Field(..., min_length=3, examples=['gpt-3.5-turbo'])
    vector_store: str = Field(..., min_length=1, examples=['/path/to/vector/store'])
...
class vLLMConfig(BaseTextConfig):
    ...
    model_path: str = Field(..., min_length=1, examples=['/path/to/vllm/model'])
...
class TransformersConfig(BaseTextConfig):
    ...
    model_path: str = Field(..., min_length=1, examples=['/path/to/transformers/model'])
...
class YandexConfig(BaseTextConfig):
    ...
    folder_id: str = Field(..., min_length=1, examples=['b1gvi7tuub11s7aeu0ul'])
    api_key: str = Field(..., min_length=32, description='API ключ для доступа к YandexGPT')
    context_length: int = 4096
    model_uri: str = Field(default='yandexgpt', examples=['yandexgpt-lite', 'yandexgpt'])
