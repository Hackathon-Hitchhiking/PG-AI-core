...
class FigureManager:
    ...
    def add_figure_shape(
        self, slide_id: int, shape_type: int, left: float, top: float, width: float, height: float, **kwargs
    ) -> GeometricShape:
        """
    ...
    def update_shape_line(
        self, slide_id: int, shape_id: int, color: tuple[int, int, int] = None, width: float = None
    ) -> None:
        """
        ...
        for slide_idx, slide in enumerate(self.pres.slides, 1):
            ...
            for shape in slide.shapes:
                if shape.shape_type == MSO_SHAPE_TYPE.AUTO_SHAPE:
                    self.parse_figure_shape(slide_idx, shape_idx, shape)
                    shape_idx += 1
                    self.shape_id_counter[slide_idx] = max(self.shape_id_counter[slide_idx], shape_idx)

    def parse_figure_shape(self, slide_id: int, shape_id: int, shape: Shape) -> GeometricShape | None:
        """
